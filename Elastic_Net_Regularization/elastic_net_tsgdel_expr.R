...
#create loss df
expr_tsg <- expr[,which(colnames(expr) %in% five_thresh_deloss_tsg)]
length(five_thresh_deloss_tsg)
...
#cohort_df <- cohort_df[which(cohort_df$CNstatus=="Deletion"),]
