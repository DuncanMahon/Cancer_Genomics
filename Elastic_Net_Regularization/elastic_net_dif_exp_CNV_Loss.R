...
#extract lost genes from cohort data
tsg_der <- cohort_df[which(cohort_df$Gene_Name %in% rownames(de_gene_table3)),]
tsg_loss <- tsg_der[which(tsg_der$CNstatus=="Loss"),]
tsg_unique <- subset(tsg_loss, select = c("Illumina_Barcode","HasBFB"))
...
tsg_arr <- array(0, dim = c(length(cohort),length(unique(tsg_loss$Gene_Name))))
t <- tsg_loss
ID <- cohort
rownames(tsg_arr) <- ID
gene_list_tsg <- unique(tsg_loss$Gene_Name)
