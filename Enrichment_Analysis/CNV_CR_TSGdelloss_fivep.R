...
#filtering cn df by TSG 5%
cn_tsg_five <- cn[which(cn$Gene_Name %in% five_thresh_deloss_tsg),]

#merging clinical and CNV data
dat.clinical.CNV <- merge(dat.clinical[,c("Illumina_Barcode","BFBtype",
                                          "HasBFB","BFBcateg")],
                          cn_tsg_five[,c("SampleID","CNstatus","Gene_Name")],
...
# all 1's again
# (only 1 individual gene < 0.05 before adjustment and not by much)
